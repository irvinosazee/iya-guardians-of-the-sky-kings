  ...
  render(){
    ...
    Object.values(GameState.PLAQUES).forEach(p => {
      ...
      if(owned){
        ...
        btn.disabled = true;
        btn.className = 'py-2 rounded-lg bg-emerald-700/60 text-emerald-100 cursor-default';
      } else if(!afford){
        btn.textContent = 'Need materials';
        btn.disabled = true;
        btn.className = 'py-2 rounded-lg bg-stone-700 text-stone-400 cursor-not-allowed';
      } else {
        btn.textContent = 'Forge plaque';
        btn.className = 'py-2 rounded-lg bg-amber-600 hover:bg-amber-500 text-stone-900 font-semibold transition';
        btn.onclick = () => {
